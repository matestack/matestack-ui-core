...
describe "Async Component", type: :feature, js: true do
  ...
  it "rerender on event behind yield" do
    ...
    page.execute_script('MatestackUiVueJs.eventHub.$emit("my_other_event")')
    expect(page).not_to have_content(content_before)
  ...
  it "rerender on event on (nested) component-level" do
    ...
    page.execute_script('MatestackUiVueJs.eventHub.$emit("some_other_event")')
    expect(page).to have_content(static_content_before)
  ...
  it "rerender on event on page-level if wrapped in app" do
    ...
    class Example::App < Matestack::Ui::Layout
      def response
    ...
    class AsyncInAppPagesController < ExampleController
      ...
      def example
        render Example::Pages::ExamplePage, matestack_layout: Example::App
      end
    end
  ...
  describe 'iterations' do
    ...
    it "rerender async components seperatly" do
      ...
      page.execute_script('MatestackUiVueJs.eventHub.$emit("my_event_0")')
      ...
      page.execute_script('MatestackUiVueJs.eventHub.$emit("multi_event")')
      (0..5).each do |item|
        expect(page).to have_content("#{item} -", count: 1)
      end
      (0..5).each do |item|
