    ...
    module Core
      module TagHelper
        ...
        VOID_TAGS.each do |tag|
          next if tag == :img  # explicitly defined below with asset path support
          define_method tag do |options = {}|
            Matestack::Ui::Core::Base.new(tag, nil, options)
          end
        end

        (TAGS - VOID_TAGS).each do |tag|
          next if tag == :a  # explicitly defined below with :path => :href support
